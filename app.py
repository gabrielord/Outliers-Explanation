...
def explanation(z_scores_df, contract_id):
    ...
    st.subheader(f"**Explication possible de l'outlier :**")
...
col_selected = st.selectbox("Sélectionnez la métrique pour analyser les outliers", opt_cols)
...
X = df_aapl[important_cols].drop(columns=col_selected)
y = df_aapl[col_selected]
...
st.write(z_scores_df.sort_values(by='Absolute Z-Score', ascending=False))
...
explanation(z_scores_df, contract_id_selected)